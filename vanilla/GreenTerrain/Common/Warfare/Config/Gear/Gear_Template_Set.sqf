...

{
	...
	if (count _nils < 1) then {
		...
		if (CTI_Log_Level >= CTI_Log_Error) then { ["DEBUG", "FILE: Common\Warfare\Config\Gear\Gear_Template_Set.sqf", format ["Template [%1] has been set with name [%2], cost [%3] and upgrade level [%4].", _forEachIndex, _label, _cost, _upgrade]] call CTI_CO_FNC_Log };
	} else {
		//todo diaglog error
		if (CTI_Log_Level >= CTI_Log_Error) then { ["ERROR", "FILE: Common\Warfare\Config\Gear\Gear_Template_Set.sqf", format ["Template [%1] contains nil items [%2].", _forEachIndex, _nils]] call CTI_CO_FNC_Log };
	};
} forEach _templates;
