...
########################
  ...
  # Description #
	...
	The core system defines the units and weapons, it's a modulable system.
	  ...
	  # The core files > Common\Warfare\Config\ #
		...
		> Gear <
			 ...
			 Vanilla gamemode will have Gear_USMC, Gear_RU, Gear_GUE called for all sides. You may create more like Gear_ACE for instance.
			  ...
			  > Each magazines list Calls "Common\Warfare\Config\Config_Magazines.sqf" ([_faction, _u, _p, _n, _o, _z] Call Compile preprocessFile "Common\Warfare\Config\Config_Magazines.sqf";)
			  ...
			  > Each weapons list Calls "Common\Warfare\Config\Config_Weapons.sqf" ([_faction, _u, _p, _n, _o, _z, _m] Call Compile preprocessFile "Common\Warfare\Config\Config_Weapons.sqf";)
			  ...
			  > Each backpacks list Calls "Common\Warfare\Config\Config_Backpack.sqf" ([_faction, _u, _p, _n, _o, _z] Call Compile preprocessFile "Common\Warfare\Config\Config_Backpack.sqf";)
	    ...
	    > Core_Groups <
			 Generic file which deals with groups which are being used in towns, only one Core_Groups may be called by side.
			  ...
			  > A list of group calls "Common\Warfare\Config\Config_Groups.sqf"
	    ...
	    > Loadout <
			 Generic file which defines the loadouts to use in the Gear Menu, more than one Loadout file may be called per side like Loadout_US & Loadout_USMC, those files are commonly called in Core_Root ones.
			  ...
			  > Magazines calls "Common\Warfare\Config\Config_SortMagazines.sqf" (_m = [_m, _side] Call Compile preprocessFile "Common\Warfare\Config\Config_SortMagazines.sqf";) which set or update the magazines, note that the returned values is a list of non-side-defined magazines used by SortWeapons.sqf (for the WF_%1_All).
		  	  > Weapons classnames are defined here.
			  > Weapons calls "Common\Warfare\Config\Config_SortWeapons.sqf" ([_u, _m, _side] Call Compile preprocessFile "Common\Warfare\Config\Config_SortWeapons.sqf";) which set or update the weapons. Note that _m is the non-side-defined magazines returned by SortMagazines.sqf
			  > Templates are defined here.
			  > Only one template per side may be used by default, see below in order to add others.
			  > Templates calls "Common\Warfare\Config\Config_SetTemplates.sqf" ([_u, _side] Call Compile preprocessFile "Common\Warfare\Config\Config_SetTemplates.sqf";), you may add an extra "false" parameter to allow multiple template usage in case of multiple Loadout_x files call per side.
