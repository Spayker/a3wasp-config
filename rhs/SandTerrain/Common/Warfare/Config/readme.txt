	  ...
	  # The core files > Common\Config\ #
	    ...
	    > Loadout <
			 Generic file which defines the loadouts to use in the Gear Menu, more than one Loadout file may be called per side like Loadout_US & Loadout_USMC, those files are commonly called in Core_Root ones.
			  ...
			  > Magazines calls "Common\Config\Config_SortMagazines.sqf" (_m = [_m, _side] Call Compile preprocessFile "Common\Config\Config_SortMagazines.sqf";) which set or update the magazines, note that the returned values is a list of non-side-defined magazines used by SortWeapons.sqf (for the WF_%1_All).
